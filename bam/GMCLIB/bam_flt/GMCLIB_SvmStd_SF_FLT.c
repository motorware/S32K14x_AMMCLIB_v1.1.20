/**
 * GMCLIB_SvmStd_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         uint32 y1 = GMCLIB_SvmStd_w_FLT(SWLIBS_3Syst_FLT y2[1], SWLIBS_2Syst_FLT u1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:06:20
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_SvmStd_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    #endif
    ssSetBusOutputObjectName(S, 1, (void *)"SWLIBS_3Syst_FLT");
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_SvmStd_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_SvmStd_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_SvmStd_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_SvmStd_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __SWLIBS_2Syst_FLTId);
        ...
        __dtBusInfo[6] = ssGetBusElementOffset(S, __SWLIBS_2Syst_FLTId, 0);
        __dtBusInfo[7] = __dtSizeInfo[1];

        /* Get information for accessing SWLIBS_2Syst_FLT.fltArg2 */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __SWLIBS_2Syst_FLTId, 1);
        __dtBusInfo[9] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__y2BUS) */
        {
            SWLIBS_3Syst_FLT* __y2BUS = (SWLIBS_3Syst_FLT*)calloc(sizeof(SWLIBS_3Syst_FLT), ssGetOutputPortWidth(S, 1));
            if (__y2BUS==NULL) {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        SWLIBS_3Syst_FLT* __y2BUS = (SWLIBS_3Syst_FLT*) ssGetPWorkValue(S, 0);
        SWLIBS_2Syst_FLT* __u1BUS = (SWLIBS_2Syst_FLT*) ssGetPWorkValue(S, 1);

        /* Assign the Simulink structure u1 to user structure __u1BUS */
        (void) memcpy(&__u1BUS[0].fltArg1, u1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__u1BUS[0].fltArg2, u1 +  __dtBusInfo[8],  __dtBusInfo[9]);

        /* Call the legacy code function */
        *y1 = GMCLIB_SvmStd_w_FLT(__y2BUS, __u1BUS);
        ...
        (void) memcpy(y2 +  __dtBusInfo[4], &__y2BUS[0].fltArg3,  __dtBusInfo[5]);
    } /* if */
}
