/**
 * GMCLIB_BetaProjection_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void GMCLIB_BetaProjection_w_FLT(SWLIBS_2Syst_FLT y1[1], SWLIBS_3Syst_FLT u1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:05:07
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_BetaProjection_SF_FLT
...
#include "gmclib.h"
#include "GMCLIB_BetaProjection_w.h"
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_BetaProjection_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_BetaProjection_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_BetaProjection_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_BetaProjection_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __SWLIBS_3Syst_FLTId);
        ...
        __dtBusInfo[4] = ssGetBusElementOffset(S, __SWLIBS_3Syst_FLTId, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing SWLIBS_3Syst_FLT.fltArg2 */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __SWLIBS_3Syst_FLTId, 1);
        __dtBusInfo[7] = __dtSizeInfo[1];

        /* Get information for accessing SWLIBS_3Syst_FLT.fltArg3 */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __SWLIBS_3Syst_FLTId, 2);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        (void) memcpy(&__u1BUS[0].fltArg1, u1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__u1BUS[0].fltArg2, u1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__u1BUS[0].fltArg3, u1 +  __dtBusInfo[8],  __dtBusInfo[9]);

        /* Call the legacy code function */
        GMCLIB_BetaProjection_w_FLT(__y1BUS, __u1BUS);
