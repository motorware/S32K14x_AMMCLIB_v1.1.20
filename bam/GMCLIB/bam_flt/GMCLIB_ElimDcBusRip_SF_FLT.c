/**
 * GMCLIB_ElimDcBusRip_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void GMCLIB_ElimDcBusRip_w_FLT(SWLIBS_2Syst_FLT y1[1], SWLIBS_2Syst_FLT u1[1], GMCLIB_ELIMDCBUSRIP_T_FLT work1[1], single u2, single u3, int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:05:40
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_ElimDcBusRip_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GMCLIB_ELIMDCBUSRIP_T_FLT", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_ElimDcBusRipInit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRipInit_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GMCLIB_ELIMDCBUSRIP_T_FLTId = ssGetDataTypeId(S, "GMCLIB_ELIMDCBUSRIP_T_FLT");
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __GMCLIB_ELIMDCBUSRIP_T_FLTId);
        ...
        /* Get information for accessing GMCLIB_ELIMDCBUSRIP_T_FLT.fltArgDcBusMsr */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GMCLIB_ELIMDCBUSRIP_T_FLTId, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing GMCLIB_ELIMDCBUSRIP_T_FLT.fltModIndex */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GMCLIB_ELIMDCBUSRIP_T_FLTId, 1);
        ...
        {
            GMCLIB_ELIMDCBUSRIP_T_FLT* __work1BUS = (GMCLIB_ELIMDCBUSRIP_T_FLT*)calloc(sizeof(GMCLIB_ELIMDCBUSRIP_T_FLT), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GMCLIB_ELIMDCBUSRIP_T_FLT* __work1BUS = (GMCLIB_ELIMDCBUSRIP_T_FLT*) ssGetPWorkValue(S, 2);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].fltArgDcBusMsr, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].fltModIndex, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);

        /* Call the legacy code function */
        GMCLIB_ElimDcBusRipInit_w_FLT(__work1BUS, *p2);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].fltArgDcBusMsr,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].fltModIndex,  __dtBusInfo[7]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        GMCLIB_ElimDcBusRip_w_FLT(__y1BUS, __u1BUS, __work1BUS, *u2, *u3, *p1);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_FLT', specification */
    {
        uint32_T chk[] = {2345000163, 1941719549, 2552895808, 1365444520};
        int_T status;
        status = CheckDataTypeChecksum(S, "GMCLIB_ELIMDCBUSRIP_T_FLT", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_FLT'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_FLT' definition has changed since the S-Function was generated");
        }
    }
}
