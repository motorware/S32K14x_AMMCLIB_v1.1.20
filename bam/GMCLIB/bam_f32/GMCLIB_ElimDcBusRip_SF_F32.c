/**
 * GMCLIB_ElimDcBusRip_SF_F32.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void GMCLIB_ElimDcBusRip_w_F32(SWLIBS_2Syst_F32 y1[1], SWLIBS_2Syst_F32 u1[1], GMCLIB_ELIMDCBUSRIP_T_F32 work1[1], int32 u2, int32 u3, int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:05:37
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_ElimDcBusRip_SF_F32
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GMCLIB_ELIMDCBUSRIP_T_F32", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_ElimDcBusRipInit_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRipInit_w_F32_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_w_F32_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_SF_F32_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_SF_F32_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GMCLIB_ELIMDCBUSRIP_T_F32Id = ssGetDataTypeId(S, "GMCLIB_ELIMDCBUSRIP_T_F32");
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __GMCLIB_ELIMDCBUSRIP_T_F32Id);
        ...
        /* Get information for accessing GMCLIB_ELIMDCBUSRIP_T_F32.f32ArgDcBusMsr */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GMCLIB_ELIMDCBUSRIP_T_F32Id, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing GMCLIB_ELIMDCBUSRIP_T_F32.f32ModIndex */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GMCLIB_ELIMDCBUSRIP_T_F32Id, 1);
        ...
        {
            GMCLIB_ELIMDCBUSRIP_T_F32* __work1BUS = (GMCLIB_ELIMDCBUSRIP_T_F32*)calloc(sizeof(GMCLIB_ELIMDCBUSRIP_T_F32), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GMCLIB_ELIMDCBUSRIP_T_F32* __work1BUS = (GMCLIB_ELIMDCBUSRIP_T_F32*) ssGetPWorkValue(S, 2);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f32ArgDcBusMsr, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].f32ModIndex, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);

        /* Call the legacy code function */
        GMCLIB_ElimDcBusRipInit_w_F32(__work1BUS, *p2);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f32ArgDcBusMsr,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].f32ModIndex,  __dtBusInfo[7]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        GMCLIB_ElimDcBusRip_w_F32(__y1BUS, __u1BUS, __work1BUS, *u2, *u3, *p1);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_F32', specification */
    {
        uint32_T chk[] = {100904927, 552158355, 659029526, 4099883653};
        int_T status;
        status = CheckDataTypeChecksum(S, "GMCLIB_ELIMDCBUSRIP_T_F32", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_F32'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_F32' definition has changed since the S-Function was generated");
        }
    }
}
