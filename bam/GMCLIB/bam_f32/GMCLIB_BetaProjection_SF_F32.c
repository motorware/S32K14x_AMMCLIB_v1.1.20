/**
 * GMCLIB_BetaProjection_SF_F32.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void GMCLIB_BetaProjection_w_F32(SWLIBS_2Syst_F32 y1[1], SWLIBS_3Syst_F32 u1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:05:04
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_BetaProjection_SF_F32
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_BetaProjection_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_BetaProjection_w_F32_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_BetaProjection_SF_F32_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_BetaProjection_SF_F32_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __SWLIBS_3Syst_F32Id);
        ...
        __dtBusInfo[4] = ssGetBusElementOffset(S, __SWLIBS_3Syst_F32Id, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing SWLIBS_3Syst_F32.f32Arg2 */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __SWLIBS_3Syst_F32Id, 1);
        __dtBusInfo[7] = __dtSizeInfo[1];

        /* Get information for accessing SWLIBS_3Syst_F32.f32Arg3 */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __SWLIBS_3Syst_F32Id, 2);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        SWLIBS_3Syst_F32* __u1BUS = (SWLIBS_3Syst_F32*) ssGetPWorkValue(S, 1);

        /* Assign the Simulink structure u1 to user structure __u1BUS */
        (void) memcpy(&__u1BUS[0].f32Arg1, u1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__u1BUS[0].f32Arg2, u1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__u1BUS[0].f32Arg3, u1 +  __dtBusInfo[8],  __dtBusInfo[9]);

        /* Call the legacy code function */
        GMCLIB_BetaProjection_w_F32(__y1BUS, __u1BUS);
