/**
 * GMCLIB_SvmSci_SF_F16.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         uint16 y1 = GMCLIB_SvmSci_w_F16(SWLIBS_3Syst_F16 y2[1], SWLIBS_2Syst_F16 u1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:06:11
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_SvmSci_SF_F16
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    #endif
    ssSetBusOutputObjectName(S, 1, (void *)"SWLIBS_3Syst_F16");
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_SvmSci_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_SvmSci_w_F16_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_SvmSci_SF_F16_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_SvmSci_SF_F16_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __SWLIBS_2Syst_F16Id);
        ...
        __dtBusInfo[6] = ssGetBusElementOffset(S, __SWLIBS_2Syst_F16Id, 0);
        __dtBusInfo[7] = __dtSizeInfo[1];

        /* Get information for accessing SWLIBS_2Syst_F16.f16Arg2 */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __SWLIBS_2Syst_F16Id, 1);
        __dtBusInfo[9] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__y2BUS) */
        {
            SWLIBS_3Syst_F16* __y2BUS = (SWLIBS_3Syst_F16*)calloc(sizeof(SWLIBS_3Syst_F16), ssGetOutputPortWidth(S, 1));
            if (__y2BUS==NULL) {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        SWLIBS_3Syst_F16* __y2BUS = (SWLIBS_3Syst_F16*) ssGetPWorkValue(S, 0);
        SWLIBS_2Syst_F16* __u1BUS = (SWLIBS_2Syst_F16*) ssGetPWorkValue(S, 1);

        /* Assign the Simulink structure u1 to user structure __u1BUS */
        (void) memcpy(&__u1BUS[0].f16Arg1, u1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__u1BUS[0].f16Arg2, u1 +  __dtBusInfo[8],  __dtBusInfo[9]);

        /* Call the legacy code function */
        *y1 = GMCLIB_SvmSci_w_F16(__y2BUS, __u1BUS);
        ...
        (void) memcpy(y2 +  __dtBusInfo[4], &__y2BUS[0].f16Arg3,  __dtBusInfo[5]);
    } /* if */
}
