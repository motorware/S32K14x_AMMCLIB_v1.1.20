/**
 * GMCLIB_ElimDcBusRip_SF_F16.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void GMCLIB_ElimDcBusRip_w_F16(SWLIBS_2Syst_F16 y1[1], SWLIBS_2Syst_F16 u1[1], GMCLIB_ELIMDCBUSRIP_T_F16 work1[1], int16 u2, int16 u3, int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:05:39
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_ElimDcBusRip_SF_F16
...
#include "gmclib.h"
#include "GMCLIB_ElimDcBusRip_w.h"
#include "gflib.h"
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GMCLIB_ELIMDCBUSRIP_T_F16", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_ElimDcBusRipInit_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRipInit_w_F16_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_w_F16_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_SF_F16_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_ElimDcBusRip_SF_F16_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GMCLIB_ELIMDCBUSRIP_T_F16Id = ssGetDataTypeId(S, "GMCLIB_ELIMDCBUSRIP_T_F16");
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __GMCLIB_ELIMDCBUSRIP_T_F16Id);
        ...
        /* Get information for accessing GMCLIB_ELIMDCBUSRIP_T_F16.f16ArgDcBusMsr */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GMCLIB_ELIMDCBUSRIP_T_F16Id, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing GMCLIB_ELIMDCBUSRIP_T_F16.f16ModIndex */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GMCLIB_ELIMDCBUSRIP_T_F16Id, 1);
        ...
        {
            GMCLIB_ELIMDCBUSRIP_T_F16* __work1BUS = (GMCLIB_ELIMDCBUSRIP_T_F16*)calloc(sizeof(GMCLIB_ELIMDCBUSRIP_T_F16), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GMCLIB_ELIMDCBUSRIP_T_F16* __work1BUS = (GMCLIB_ELIMDCBUSRIP_T_F16*) ssGetPWorkValue(S, 2);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f16ArgDcBusMsr, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].f16ModIndex, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);

        /* Call the legacy code function */
        GMCLIB_ElimDcBusRipInit_w_F16(__work1BUS, *p2);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f16ArgDcBusMsr,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].f16ModIndex,  __dtBusInfo[7]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        GMCLIB_ElimDcBusRip_w_F16(__y1BUS, __u1BUS, __work1BUS, *u2, *u3, *p1);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_F16', specification */
    {
        uint32_T chk[] = {636925632, 4102607073, 3052359902, 3487767793};
        int_T status;
        status = CheckDataTypeChecksum(S, "GMCLIB_ELIMDCBUSRIP_T_F16", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_F16'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GMCLIB_ELIMDCBUSRIP_T_F16' definition has changed since the S-Function was generated");
        }
    }
}
