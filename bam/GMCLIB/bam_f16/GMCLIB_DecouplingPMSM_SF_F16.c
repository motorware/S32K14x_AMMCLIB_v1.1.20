/**
 * GMCLIB_DecouplingPMSM_SF_F16.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void GMCLIB_DecouplingPMSM_w_F16(SWLIBS_2Syst_F16 u1[1], SWLIBS_2Syst_F16 u2[1], int16 u3, int16 u4, int16 u5, int16 u6, int16 u7, int32 p1, GMCLIB_DECOUPLINGPMSM_T_F16 work1[1], SWLIBS_2Syst_F16 y1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:05:29
 *
 */
...
#define S_FUNCTION_NAME  GMCLIB_DecouplingPMSM_SF_F16
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GMCLIB_DECOUPLINGPMSM_T_F16", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GMCLIB_DecouplingPMSMInit_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GMCLIB_DecouplingPMSM_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_DecouplingPMSMInit_w_F16_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GMCLIB_DecouplingPMSM_w_F16_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_DecouplingPMSM_SF_F16_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GMCLIB_DecouplingPMSM_SF_F16_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GMCLIB_DECOUPLINGPMSM_T_F16Id = ssGetDataTypeId(S, "GMCLIB_DECOUPLINGPMSM_T_F16");
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __GMCLIB_DECOUPLINGPMSM_T_F16Id);
        ...
        /* Get information for accessing GMCLIB_DECOUPLINGPMSM_T_F16.f16Kd */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GMCLIB_DECOUPLINGPMSM_T_F16Id, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing GMCLIB_DECOUPLINGPMSM_T_F16.s16KdShift */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GMCLIB_DECOUPLINGPMSM_T_F16Id, 1);
        __dtBusInfo[7] = __dtSizeInfo[1];

        /* Get information for accessing GMCLIB_DECOUPLINGPMSM_T_F16.f16Kq */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __GMCLIB_DECOUPLINGPMSM_T_F16Id, 2);
        __dtBusInfo[9] = __dtSizeInfo[1];

        /* Get information for accessing GMCLIB_DECOUPLINGPMSM_T_F16.s16KqShift */
        __dtBusInfo[10] = ssGetBusElementOffset(S, __GMCLIB_DECOUPLINGPMSM_T_F16Id, 3);
        ...
        {
            GMCLIB_DECOUPLINGPMSM_T_F16* __work1BUS = (GMCLIB_DECOUPLINGPMSM_T_F16*)calloc(sizeof(GMCLIB_DECOUPLINGPMSM_T_F16), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GMCLIB_DECOUPLINGPMSM_T_F16* __work1BUS = (GMCLIB_DECOUPLINGPMSM_T_F16*) ssGetPWorkValue(S, 3);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f16Kd, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].s16KdShift, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__work1BUS[0].f16Kq, work1 +  __dtBusInfo[8],  __dtBusInfo[9]);
        (void) memcpy(&__work1BUS[0].s16KqShift, work1 +  __dtBusInfo[10],  __dtBusInfo[11]);

        /* Call the legacy code function */
        GMCLIB_DecouplingPMSMInit_w_F16(*p2, *p3, *p4, *p5, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f16Kd,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].s16KdShift,  __dtBusInfo[7]);
        (void) memcpy(work1 +  __dtBusInfo[8], &__work1BUS[0].f16Kq,  __dtBusInfo[9]);
        (void) memcpy(work1 +  __dtBusInfo[10], &__work1BUS[0].s16KqShift,  __dtBusInfo[11]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        SWLIBS_2Syst_F16* __u1BUS = (SWLIBS_2Syst_F16*) ssGetPWorkValue(S, 1);
        SWLIBS_2Syst_F16* __u2BUS = (SWLIBS_2Syst_F16*) ssGetPWorkValue(S, 2);
        GMCLIB_DECOUPLINGPMSM_T_F16* __work1BUS = (GMCLIB_DECOUPLINGPMSM_T_F16*) ssGetPWorkValue(S, 3);
        ...
        /* Call the legacy code function */
        GMCLIB_DecouplingPMSM_w_F16(__u1BUS, __u2BUS, *u3, *u4, *u5, *u6, *u7, *p1, __work1BUS, __y1BUS);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GMCLIB_DECOUPLINGPMSM_T_F16', specification */
    {
        uint32_T chk[] = {3589216880, 3632913113, 1443923502, 3463884228};
        int_T status;
        status = CheckDataTypeChecksum(S, "GMCLIB_DECOUPLINGPMSM_T_F16", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GMCLIB_DECOUPLINGPMSM_T_F16'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GMCLIB_DECOUPLINGPMSM_T_F16' definition has changed since the S-Function was generated");
        }
    }
}
