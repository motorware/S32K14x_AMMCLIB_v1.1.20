/**
 * AMCLIB_SpeedLoop_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void AMCLIB_SpeedLoop_w_FLT(single u1, single u2, GDFLIB_FILTER_MA_T_FLT work1[1], GFLIB_CONTROLLER_PIAW_P_T_FLT work2[1], GFLIB_RAMP_T_FLT work3[1], int32 work4[1], single u3, single u4, single u5, single u6, single u7, single u8, single u9, single u10, single u11, single u12, single u13, single u14, single u15, single u16, SWLIBS_2Syst_FLT y1[1], uint16 y2[1], single y3[1], single y4[1], single y5[1], single y6[1], single y7[1], single y8[1], single y9[1], int32 p1, int32 p2, int32 p3)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 20:59:33
 *
 */
...
#define S_FUNCTION_NAME  AMCLIB_SpeedLoop_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "AMCLIB_SpeedLoopInit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "AMCLIB_SpeedLoop_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "AMCLIB_SpeedLoopInit_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "AMCLIB_SpeedLoop_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "AMCLIB_SpeedLoop_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "AMCLIB_SpeedLoop_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        {
            GFLIB_CONTROLLER_PIAW_P_T_FLT* __work2BUS = (GFLIB_CONTROLLER_PIAW_P_T_FLT*)calloc(sizeof(GFLIB_CONTROLLER_PIAW_P_T_FLT), ssGetDWorkWidth(S, 1));
            if (__work2BUS==NULL) {
        ...
        {
            GFLIB_RAMP_T_FLT* __work3BUS = (GFLIB_RAMP_T_FLT*)calloc(sizeof(GFLIB_RAMP_T_FLT), ssGetDWorkWidth(S, 2));
            if (__work3BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GDFLIB_FILTER_MA_T_FLT* __work1BUS = (GDFLIB_FILTER_MA_T_FLT*) ssGetPWorkValue(S, 1);
        GFLIB_CONTROLLER_PIAW_P_T_FLT* __work2BUS = (GFLIB_CONTROLLER_PIAW_P_T_FLT*) ssGetPWorkValue(S, 2);
        GFLIB_RAMP_T_FLT* __work3BUS = (GFLIB_RAMP_T_FLT*) ssGetPWorkValue(S, 3);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].fltAcc, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].fltLambda, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);

        /* Assign the Simulink structure work2 to user structure __work2BUS */
        (void) memcpy(&__work2BUS[0].fltPropGain, work2 +  __dtBusInfo[8],  __dtBusInfo[9]);
        (void) memcpy(&__work2BUS[0].fltIntegGain, work2 +  __dtBusInfo[10],  __dtBusInfo[11]);
        (void) memcpy(&__work2BUS[0].fltLowerLimit, work2 +  __dtBusInfo[12],  __dtBusInfo[13]);
        (void) memcpy(&__work2BUS[0].fltUpperLimit, work2 +  __dtBusInfo[14],  __dtBusInfo[15]);
        (void) memcpy(&__work2BUS[0].fltIntegPartK_1, work2 +  __dtBusInfo[16],  __dtBusInfo[17]);
        (void) memcpy(&__work2BUS[0].fltInK_1, work2 +  __dtBusInfo[18],  __dtBusInfo[19]);
        (void) memcpy(&__work2BUS[0].u16LimitFlag, work2 +  __dtBusInfo[20],  __dtBusInfo[21]);

        /* Assign the Simulink structure work3 to user structure __work3BUS */
        (void) memcpy(&__work3BUS[0].fltState, work3 +  __dtBusInfo[22],  __dtBusInfo[23]);
        (void) memcpy(&__work3BUS[0].fltRampUp, work3 +  __dtBusInfo[24],  __dtBusInfo[25]);
        (void) memcpy(&__work3BUS[0].fltRampDown, work3 +  __dtBusInfo[26],  __dtBusInfo[27]);

        /* Call the legacy code function */
        AMCLIB_SpeedLoopInit_w_FLT(__work1BUS, __work2BUS, __work3BUS, work4, *p4, *p5, *p6, *p7, *p8, *p9, *p10, *p1, *p2, *p3);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].fltAcc,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].fltLambda,  __dtBusInfo[7]);

        /* Assign the user structure __work2BUS to the Simulink structure work2 */
        (void) memcpy(work2 +  __dtBusInfo[8], &__work2BUS[0].fltPropGain,  __dtBusInfo[9]);
        (void) memcpy(work2 +  __dtBusInfo[10], &__work2BUS[0].fltIntegGain,  __dtBusInfo[11]);
        (void) memcpy(work2 +  __dtBusInfo[12], &__work2BUS[0].fltLowerLimit,  __dtBusInfo[13]);
        (void) memcpy(work2 +  __dtBusInfo[14], &__work2BUS[0].fltUpperLimit,  __dtBusInfo[15]);
        (void) memcpy(work2 +  __dtBusInfo[16], &__work2BUS[0].fltIntegPartK_1,  __dtBusInfo[17]);
        (void) memcpy(work2 +  __dtBusInfo[18], &__work2BUS[0].fltInK_1,  __dtBusInfo[19]);
        (void) memcpy(work2 +  __dtBusInfo[20], &__work2BUS[0].u16LimitFlag,  __dtBusInfo[21]);

        /* Assign the user structure __work3BUS to the Simulink structure work3 */
        (void) memcpy(work3 +  __dtBusInfo[22], &__work3BUS[0].fltState,  __dtBusInfo[23]);
        (void) memcpy(work3 +  __dtBusInfo[24], &__work3BUS[0].fltRampUp,  __dtBusInfo[25]);
        (void) memcpy(work3 +  __dtBusInfo[26], &__work3BUS[0].fltRampDown,  __dtBusInfo[27]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        AMCLIB_SpeedLoop_w_FLT(*u1, *u2, __work1BUS, __work2BUS, __work3BUS, work4, *u3, *u4, *u5, *u6, *u7, *u8, *u9, *u10, *u11, *u12, *u13, *u14, *u15, *u16, __y1BUS, y2, y3, y4, y5, y6, y7, y8, y9, *p1, *p2, *p3);
