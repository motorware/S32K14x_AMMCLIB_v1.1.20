/**
 * GFLIB_AtanYXShifted_SF_F32.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         int32  y1 = GFLIB_AtanYXShifted_w_F32(int32 u1, int32 u2, int32 u3, int32 u4, int32 u5, int32 u6, int32 u7, GFLIB_ATANYXSHIFTED_T_F32 work1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:01:13
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_AtanYXShifted_SF_F32
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_ATANYXSHIFTED_T_F32", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_AtanYXShifted_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_AtanYXShifted_w_F32_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_AtanYXShifted_SF_F32_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_AtanYXShifted_SF_F32_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GFLIB_ATANYXSHIFTED_T_F32Id = ssGetDataTypeId(S, "GFLIB_ATANYXSHIFTED_T_F32");
        DTypeId __int32Id = ssGetDataTypeId(S, "int32");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_ATANYXSHIFTED_T_F32Id);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __int32Id);

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_F32.f32Ky */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_F32Id, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_F32.f32Kx */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_F32Id, 1);
        __dtBusInfo[3] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_F32.s32Ny */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_F32Id, 2);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_F32.s32Nx */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_F32Id, 3);
        __dtBusInfo[7] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_F32.f32ThetaAdj */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_F32Id, 4);
        __dtBusInfo[9] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_ATANYXSHIFTED_T_F32* __work1BUS = (GFLIB_ATANYXSHIFTED_T_F32*)calloc(sizeof(GFLIB_ATANYXSHIFTED_T_F32), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_ATANYXSHIFTED_T_F32* __work1BUS = (GFLIB_ATANYXSHIFTED_T_F32*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f32Ky, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__work1BUS[0].f32Kx, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].s32Ny, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].s32Nx, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__work1BUS[0].f32ThetaAdj, work1 +  __dtBusInfo[8],  __dtBusInfo[9]);

        /* Call the legacy code function */
        *y1 = GFLIB_AtanYXShifted_w_F32(*u1, *u2, *u3, *u4, *u5, *u6, *u7, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].f32Ky,  __dtBusInfo[1]);
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].f32Kx,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].s32Ny,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].s32Nx,  __dtBusInfo[7]);
        (void) memcpy(work1 +  __dtBusInfo[8], &__work1BUS[0].f32ThetaAdj,  __dtBusInfo[9]);
    } /* if */
}
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GFLIB_ATANYXSHIFTED_T_F32', specification */
    {
        uint32_T chk[] = {2911010238, 1121303889, 3579746779, 1799243685};
        int_T status;
        status = CheckDataTypeChecksum(S, "GFLIB_ATANYXSHIFTED_T_F32", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GFLIB_ATANYXSHIFTED_T_F32'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GFLIB_ATANYXSHIFTED_T_F32' definition has changed since the S-Function was generated");
        }
    }
}
