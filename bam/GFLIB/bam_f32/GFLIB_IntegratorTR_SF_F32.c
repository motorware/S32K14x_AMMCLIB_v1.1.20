/**
 * GFLIB_IntegratorTR_SF_F32.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         int32  y1 = GFLIB_IntegratorTR_w_F32(int32 u1, GFLIB_INTEGRATOR_TR_T_F32 work1[1], int32 work2[1], int32 u2, int32 u3, uint16 u4, int32 u5, int32 u6, int32 y2[1], int32 y3[1], int32 p1, int32 p2)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:02:54
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_IntegratorTR_SF_F32
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_INTEGRATOR_TR_T_F32", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_IntegratorTRInit_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_IntegratorTR_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_IntegratorTRInit_w_F32_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_IntegratorTR_w_F32_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_IntegratorTR_SF_F32_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_IntegratorTR_SF_F32_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_INTEGRATOR_TR_T_F32Id);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __int32Id);
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __uint16Id);

        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F32.f32State */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F32Id, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F32.f32InK1 */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F32Id, 1);
        __dtBusInfo[3] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F32.f32C1 */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F32Id, 2);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F32.u16NShift */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F32Id, 3);
        __dtBusInfo[7] = __dtSizeInfo[2];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_INTEGRATOR_TR_T_F32* __work1BUS = (GFLIB_INTEGRATOR_TR_T_F32*)calloc(sizeof(GFLIB_INTEGRATOR_TR_T_F32), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_INTEGRATOR_TR_T_F32* __work1BUS = (GFLIB_INTEGRATOR_TR_T_F32*) ssGetPWorkValue(S, 0);
        ...
        (void) memcpy(&__work1BUS[0].f32InK1, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].f32C1, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].u16NShift, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);

        /* Call the legacy code function */
        GFLIB_IntegratorTRInit_w_F32(*p3, *p4, __work1BUS, work2, *p2);
        ...
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].f32InK1,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f32C1,  __dtBusInfo[5]);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_IntegratorTR_w_F32(*u1, __work1BUS, work2, *u2, *u3, *u4, *u5, *u6, y2, y3, *p1, *p2);
