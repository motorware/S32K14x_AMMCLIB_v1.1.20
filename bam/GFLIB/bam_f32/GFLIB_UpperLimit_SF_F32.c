/**
 * GFLIB_UpperLimit_SF_F32.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         int32 y1 = GFLIB_UpperLimit_w_F32(int32 u1, GFLIB_UPPERLIMIT_T_F32 work1[1], int32 u2, int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:04:27
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_UpperLimit_SF_F32
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_UPPERLIMIT_T_F32", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_UpperLimitInit_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_UpperLimit_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_UpperLimitInit_w_F32_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_UpperLimit_w_F32_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_UpperLimit_SF_F32_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_UpperLimit_SF_F32_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GFLIB_UPPERLIMIT_T_F32Id = ssGetDataTypeId(S, "GFLIB_UPPERLIMIT_T_F32");
        DTypeId __int32Id = ssGetDataTypeId(S, "int32");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_UPPERLIMIT_T_F32Id);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __int32Id);

        /* Get information for accessing GFLIB_UPPERLIMIT_T_F32.f32UpperLimit */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_UPPERLIMIT_T_F32Id, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_UPPERLIMIT_T_F32* __work1BUS = (GFLIB_UPPERLIMIT_T_F32*)calloc(sizeof(GFLIB_UPPERLIMIT_T_F32), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_UPPERLIMIT_T_F32* __work1BUS = (GFLIB_UPPERLIMIT_T_F32*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f32UpperLimit, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);

        /* Call the legacy code function */
        GFLIB_UpperLimitInit_w_F32(*p2, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].f32UpperLimit,  __dtBusInfo[1]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_UpperLimit_w_F32(*u1, __work1BUS, *u2, *p1);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GFLIB_UPPERLIMIT_T_F32', specification */
    {
        uint32_T chk[] = {2485155314, 473428499, 2390277797, 1266238233};
        int_T status;
        status = CheckDataTypeChecksum(S, "GFLIB_UPPERLIMIT_T_F32", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GFLIB_UPPERLIMIT_T_F32'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GFLIB_UPPERLIMIT_T_F32' definition has changed since the S-Function was generated");
        }
    }
}
