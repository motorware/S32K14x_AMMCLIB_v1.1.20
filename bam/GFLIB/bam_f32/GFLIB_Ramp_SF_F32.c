 ...
 *
 *         int32  y1 = GFLIB_Ramp_w_F32(int32 u1, GFLIB_RAMP_T_F32 work1[1], int32 u2, int32 u3, int32 u4, int32 y2[1], int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:03:41
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_Ramp_SF_F32
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_RampInit_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_Ramp_w_F32", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_RampInit_w_F32_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_Ramp_w_F32_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Ramp_SF_F32_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Ramp_SF_F32_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_RAMP_T_F32* __work1BUS = (GFLIB_RAMP_T_F32*) ssGetPWorkValue(S, 0);
        ...
        (void) memcpy(&__work1BUS[0].f32RampUp, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].f32RampDown, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);

        /* Call the legacy code function */
        GFLIB_RampInit_w_F32(*p2, *p3, __work1BUS);
        ...
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].f32RampUp,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f32RampDown,  __dtBusInfo[5]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_Ramp_w_F32(*u1, __work1BUS, *u2, *u3, *u4, y2, *p1);
