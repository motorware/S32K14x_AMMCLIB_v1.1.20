 ...
 *
 *         single y1 = GFLIB_Ramp_w_FLT(single u1, GFLIB_RAMP_T_FLT work1[1], single u2, single u3, single u4, single y2[1], int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:03:45
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_Ramp_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_RampInit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_Ramp_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_RampInit_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_Ramp_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Ramp_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Ramp_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_RAMP_T_FLT* __work1BUS = (GFLIB_RAMP_T_FLT*) ssGetPWorkValue(S, 0);
        ...
        (void) memcpy(&__work1BUS[0].fltRampUp, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].fltRampDown, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);

        /* Call the legacy code function */
        GFLIB_RampInit_w_FLT(*p2, *p3, __work1BUS);
        ...
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].fltRampUp,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].fltRampDown,  __dtBusInfo[5]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_Ramp_w_FLT(*u1, __work1BUS, *u2, *u3, *u4, y2, *p1);
