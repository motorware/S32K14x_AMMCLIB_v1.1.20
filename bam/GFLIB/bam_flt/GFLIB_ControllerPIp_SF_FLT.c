/**
 * GFLIB_ControllerPIp_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         single y1 = GFLIB_ControllerPIp_w_FLT(single u1, GFLIB_CONTROLLER_PI_P_T_FLT work1[1], int32 work2[1], single u2, single u3, single u4, single u5, single u6, single y2[1], single y3[1], int32 p1, int32 p2)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:02:09
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_ControllerPIp_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_CONTROLLER_PI_P_T_FLT", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_ControllerPIpInit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_ControllerPIp_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_ControllerPIpInit_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_ControllerPIp_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_ControllerPIp_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_ControllerPIp_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GFLIB_CONTROLLER_PI_P_T_FLTId = ssGetDataTypeId(S, "GFLIB_CONTROLLER_PI_P_T_FLT");
        DTypeId __singleId = ssGetDataTypeId(S, "single");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_CONTROLLER_PI_P_T_FLTId);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __singleId);

        /* Get information for accessing GFLIB_CONTROLLER_PI_P_T_FLT.fltPropGain */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_CONTROLLER_PI_P_T_FLTId, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_CONTROLLER_PI_P_T_FLT.fltIntegGain */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GFLIB_CONTROLLER_PI_P_T_FLTId, 1);
        __dtBusInfo[3] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_CONTROLLER_PI_P_T_FLT.fltIntegPartK_1 */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GFLIB_CONTROLLER_PI_P_T_FLTId, 2);
        __dtBusInfo[5] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_CONTROLLER_PI_P_T_FLT.fltInK_1 */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GFLIB_CONTROLLER_PI_P_T_FLTId, 3);
        __dtBusInfo[7] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_CONTROLLER_PI_P_T_FLT* __work1BUS = (GFLIB_CONTROLLER_PI_P_T_FLT*)calloc(sizeof(GFLIB_CONTROLLER_PI_P_T_FLT), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_CONTROLLER_PI_P_T_FLT* __work1BUS = (GFLIB_CONTROLLER_PI_P_T_FLT*) ssGetPWorkValue(S, 0);
        ...
        (void) memcpy(&__work1BUS[0].fltIntegPartK_1, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].fltInK_1, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);

        /* Call the legacy code function */
        GFLIB_ControllerPIpInit_w_FLT(*p3, *p4, __work1BUS, work2, *p2);
        ...
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].fltIntegPartK_1,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].fltInK_1,  __dtBusInfo[7]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_ControllerPIp_w_FLT(*u1, __work1BUS, work2, *u2, *u3, *u4, *u5, *u6, y2, y3, *p1, *p2);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GFLIB_CONTROLLER_PI_P_T_FLT', specification */
    {
        uint32_T chk[] = {3897551777, 1259126355, 837917552, 611799753};
        int_T status;
        status = CheckDataTypeChecksum(S, "GFLIB_CONTROLLER_PI_P_T_FLT", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GFLIB_CONTROLLER_PI_P_T_FLT'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GFLIB_CONTROLLER_PI_P_T_FLT' definition has changed since the S-Function was generated");
        }
    }
}
