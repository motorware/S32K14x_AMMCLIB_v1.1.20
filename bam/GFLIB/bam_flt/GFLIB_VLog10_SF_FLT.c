/**
 * GFLIB_VLog10_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void GFLIB_VLog10_w_FLT(single u1[], single y1[size(u1,1)])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:04:35
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_VLog10_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    {
        int_T y1Width = DYNAMICALLY_SIZED;
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_VLog10_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
    } /* if */
...
static void mdlSetOutputPortDimensionInfo(SimStruct *S, int_T portIndex, const DimsInfo_T *dimsInfo)
{
    ...
    if(!ssSetOutputPortDimensionInfo(S, portIndex, dimsInfo)) return;
    {
        ...
        if (y1Width!=DYNAMICALLY_SIZED && y1Width < 1) { ssSetErrorStatus(S, "Output 1: the current width is < 1"); return; }
        if (y1Width != DYNAMICALLY_SIZED) {
            if ((dimsInfo->numDims != 1) || (dimsInfo->width != y1Width)) {
                ssSetErrorStatus(S, "Invalid output port width. The output signal 1 must be a 1D signal.");
            }
        } /* if */
    }
}
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    GFLIB_VLog10_w_FLT(u1, ssGetCurrentInputPortWidth(S, 0), y1);
}
