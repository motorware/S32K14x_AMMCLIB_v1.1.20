/**
 * GFLIB_AtanYXShifted_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         single y1 = GFLIB_AtanYXShifted_w_FLT(single u1, single u2, single u3, single u4, single u5, GFLIB_ATANYXSHIFTED_T_FLT work1[1])
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:01:17
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_AtanYXShifted_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_ATANYXSHIFTED_T_FLT", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_AtanYXShifted_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_AtanYXShifted_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_AtanYXShifted_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_AtanYXShifted_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GFLIB_ATANYXSHIFTED_T_FLTId = ssGetDataTypeId(S, "GFLIB_ATANYXSHIFTED_T_FLT");
        DTypeId __singleId = ssGetDataTypeId(S, "single");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_ATANYXSHIFTED_T_FLTId);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __singleId);

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_FLT.fltKy */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_FLTId, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_FLT.fltKx */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_FLTId, 1);
        __dtBusInfo[3] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_ATANYXSHIFTED_T_FLT.fltThetaAdj */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GFLIB_ATANYXSHIFTED_T_FLTId, 2);
        __dtBusInfo[5] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_ATANYXSHIFTED_T_FLT* __work1BUS = (GFLIB_ATANYXSHIFTED_T_FLT*)calloc(sizeof(GFLIB_ATANYXSHIFTED_T_FLT), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_ATANYXSHIFTED_T_FLT* __work1BUS = (GFLIB_ATANYXSHIFTED_T_FLT*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].fltKy, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__work1BUS[0].fltKx, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].fltThetaAdj, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);

        /* Call the legacy code function */
        *y1 = GFLIB_AtanYXShifted_w_FLT(*u1, *u2, *u3, *u4, *u5, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].fltKy,  __dtBusInfo[1]);
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].fltKx,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].fltThetaAdj,  __dtBusInfo[5]);
    } /* if */
}
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GFLIB_ATANYXSHIFTED_T_FLT', specification */
    {
        uint32_T chk[] = {973096006, 3295017064, 1918543584, 3573610949};
        int_T status;
        status = CheckDataTypeChecksum(S, "GFLIB_ATANYXSHIFTED_T_FLT", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GFLIB_ATANYXSHIFTED_T_FLT'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GFLIB_ATANYXSHIFTED_T_FLT' definition has changed since the S-Function was generated");
        }
    }
}
