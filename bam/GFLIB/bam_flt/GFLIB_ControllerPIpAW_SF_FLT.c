/**
 * GFLIB_ControllerPIpAW_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         single y1 = GFLIB_ControllerPIpAW_w_FLT(single u1, GFLIB_CONTROLLER_PIAW_P_T_FLT work1[1], int32 work2[1], single u2, single u3, single u4, single u5, single u6, single u7, single u8, uint16 y2[1], single y3[1], single y4[1], int32 p1, int32 p2)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:01:58
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_ControllerPIpAW_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_ControllerPIpAWInit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_ControllerPIpAW_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_ControllerPIpAWInit_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_ControllerPIpAW_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_ControllerPIpAW_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_ControllerPIpAW_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_CONTROLLER_PIAW_P_T_FLTId);
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_CONTROLLER_PIAW_P_T_FLT* __work1BUS = (GFLIB_CONTROLLER_PIAW_P_T_FLT*) ssGetPWorkValue(S, 0);
        ...
        (void) memcpy(&__work1BUS[0].fltLowerLimit, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].fltUpperLimit, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__work1BUS[0].fltIntegPartK_1, work1 +  __dtBusInfo[8],  __dtBusInfo[9]);
        (void) memcpy(&__work1BUS[0].fltInK_1, work1 +  __dtBusInfo[10],  __dtBusInfo[11]);
        (void) memcpy(&__work1BUS[0].u16LimitFlag, work1 +  __dtBusInfo[12],  __dtBusInfo[13]);

        /* Call the legacy code function */
        GFLIB_ControllerPIpAWInit_w_FLT(*p3, *p4, *p5, *p6, __work1BUS, work2, *p2);
        ...
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].fltLowerLimit,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].fltUpperLimit,  __dtBusInfo[7]);
        (void) memcpy(work1 +  __dtBusInfo[8], &__work1BUS[0].fltIntegPartK_1,  __dtBusInfo[9]);
        (void) memcpy(work1 +  __dtBusInfo[10], &__work1BUS[0].fltInK_1,  __dtBusInfo[11]);
        (void) memcpy(work1 +  __dtBusInfo[12], &__work1BUS[0].u16LimitFlag,  __dtBusInfo[13]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_ControllerPIpAW_w_FLT(*u1, __work1BUS, work2, *u2, *u3, *u4, *u5, *u6, *u7, *u8, y2, y3, y4, *p1, *p2);
