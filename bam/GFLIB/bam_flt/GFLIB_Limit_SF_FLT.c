/**
 * GFLIB_Limit_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         single y1 = GFLIB_Limit_w_FLT(single u1, GFLIB_LIMIT_T_FLT work1[1], single u2, single u3, int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:03:07
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_Limit_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_LIMIT_T_FLT", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_LimitInit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_Limit_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_LimitInit_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_Limit_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Limit_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Limit_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GFLIB_LIMIT_T_FLTId = ssGetDataTypeId(S, "GFLIB_LIMIT_T_FLT");
        DTypeId __singleId = ssGetDataTypeId(S, "single");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_LIMIT_T_FLTId);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __singleId);

        /* Get information for accessing GFLIB_LIMIT_T_FLT.fltLowerLimit */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_LIMIT_T_FLTId, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_LIMIT_T_FLT.fltUpperLimit */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GFLIB_LIMIT_T_FLTId, 1);
        __dtBusInfo[3] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_LIMIT_T_FLT* __work1BUS = (GFLIB_LIMIT_T_FLT*)calloc(sizeof(GFLIB_LIMIT_T_FLT), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_LIMIT_T_FLT* __work1BUS = (GFLIB_LIMIT_T_FLT*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].fltLowerLimit, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__work1BUS[0].fltUpperLimit, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);

        /* Call the legacy code function */
        GFLIB_LimitInit_w_FLT(*p2, *p3, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].fltLowerLimit,  __dtBusInfo[1]);
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].fltUpperLimit,  __dtBusInfo[3]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_Limit_w_FLT(*u1, __work1BUS, *u2, *u3, *p1);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GFLIB_LIMIT_T_FLT', specification */
    {
        uint32_T chk[] = {859392863, 1805581857, 224048387, 3634149798};
        int_T status;
        status = CheckDataTypeChecksum(S, "GFLIB_LIMIT_T_FLT", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GFLIB_LIMIT_T_FLT'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GFLIB_LIMIT_T_FLT' definition has changed since the S-Function was generated");
        }
    }
}
