 ...
 *
 *         int16  y1 = GFLIB_Ramp_w_F16(int16 u1, GFLIB_RAMP_T_F16 work1[1], int16 u2, int16 u3, int16 u4, int16 y2[1], int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:03:43
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_Ramp_SF_F16
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_RAMP_T_F16", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_RampInit_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_Ramp_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_RampInit_w_F16_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_Ramp_w_F16_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Ramp_SF_F16_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_Ramp_SF_F16_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GFLIB_RAMP_T_F16Id = ssGetDataTypeId(S, "GFLIB_RAMP_T_F16");
        DTypeId __int16Id = ssGetDataTypeId(S, "int16");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_RAMP_T_F16Id);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __int16Id);

        /* Get information for accessing GFLIB_RAMP_T_F16.f16State */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_RAMP_T_F16Id, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_RAMP_T_F16.f16RampUp */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GFLIB_RAMP_T_F16Id, 1);
        __dtBusInfo[3] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_RAMP_T_F16.f16RampDown */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GFLIB_RAMP_T_F16Id, 2);
        __dtBusInfo[5] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_RAMP_T_F16* __work1BUS = (GFLIB_RAMP_T_F16*)calloc(sizeof(GFLIB_RAMP_T_F16), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_RAMP_T_F16* __work1BUS = (GFLIB_RAMP_T_F16*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f16State, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__work1BUS[0].f16RampUp, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].f16RampDown, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);

        /* Call the legacy code function */
        GFLIB_RampInit_w_F16(*p2, *p3, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].f16State,  __dtBusInfo[1]);
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].f16RampUp,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f16RampDown,  __dtBusInfo[5]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_Ramp_w_F16(*u1, __work1BUS, *u2, *u3, *u4, y2, *p1);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GFLIB_RAMP_T_F16', specification */
    {
        uint32_T chk[] = {3498709237, 1733965102, 1454929276, 2276284180};
        int_T status;
        status = CheckDataTypeChecksum(S, "GFLIB_RAMP_T_F16", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GFLIB_RAMP_T_F16'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GFLIB_RAMP_T_F16' definition has changed since the S-Function was generated");
        }
    }
}
