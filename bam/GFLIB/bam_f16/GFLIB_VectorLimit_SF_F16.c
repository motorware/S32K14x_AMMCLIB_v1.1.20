/**
 * GFLIB_VectorLimit_SF_F16.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         boolean y1 = GFLIB_VectorLimit_w_F16(SWLIBS_2Syst_F16 y2[1], SWLIBS_2Syst_F16 u1[1], GFLIB_VECTORLIMIT_T_F16 work1[1], int16 u2, int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:04:49
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_VectorLimit_SF_F16
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_VECTORLIMIT_T_F16", &dataTypeIdReg);
    ...
    #endif
    ssSetBusOutputObjectName(S, 1, (void *)"SWLIBS_2Syst_F16");
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_VectorLimitInit_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_VectorLimit_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_VectorLimitInit_w_F16_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_VectorLimit_w_F16_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_VectorLimit_SF_F16_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_VectorLimit_SF_F16_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GFLIB_VECTORLIMIT_T_F16Id = ssGetDataTypeId(S, "GFLIB_VECTORLIMIT_T_F16");
        ...
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __GFLIB_VECTORLIMIT_T_F16Id);
        ...
        /* Get information for accessing GFLIB_VECTORLIMIT_T_F16.f16Limit */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GFLIB_VECTORLIMIT_T_F16Id, 0);
        __dtBusInfo[5] = __dtSizeInfo[1];


        /* Alloc memory for the pwork 1 (__y2BUS) */
        {
            SWLIBS_2Syst_F16* __y2BUS = (SWLIBS_2Syst_F16*)calloc(sizeof(SWLIBS_2Syst_F16), ssGetOutputPortWidth(S, 1));
            if (__y2BUS==NULL) {
        ...
        {
            GFLIB_VECTORLIMIT_T_F16* __work1BUS = (GFLIB_VECTORLIMIT_T_F16*)calloc(sizeof(GFLIB_VECTORLIMIT_T_F16), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_VECTORLIMIT_T_F16* __work1BUS = (GFLIB_VECTORLIMIT_T_F16*) ssGetPWorkValue(S, 2);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f16Limit, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);

        /* Call the legacy code function */
        GFLIB_VectorLimitInit_w_F16(*p2, __work1BUS);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f16Limit,  __dtBusInfo[5]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        SWLIBS_2Syst_F16* __y2BUS = (SWLIBS_2Syst_F16*) ssGetPWorkValue(S, 0);
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_VectorLimit_w_F16(__y2BUS, __u1BUS, __work1BUS, *u2, *p1);

        /* Assign the user structure __y2BUS to the Simulink structure y2 */
        (void) memcpy(y2 +  __dtBusInfo[0], &__y2BUS[0].f16Arg1,  __dtBusInfo[1]);
        (void) memcpy(y2 +  __dtBusInfo[2], &__y2BUS[0].f16Arg2,  __dtBusInfo[3]);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify Bus/StructType 'GFLIB_VECTORLIMIT_T_F16', specification */
    {
        uint32_T chk[] = {4613480, 4174354630, 1604822973, 2524253682};
        int_T status;
        status = CheckDataTypeChecksum(S, "GFLIB_VECTORLIMIT_T_F16", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GFLIB_VECTORLIMIT_T_F16'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GFLIB_VECTORLIMIT_T_F16' definition has changed since the S-Function was generated");
        }
    }
}
