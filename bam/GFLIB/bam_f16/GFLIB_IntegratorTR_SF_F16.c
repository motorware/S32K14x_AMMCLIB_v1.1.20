/**
 * GFLIB_IntegratorTR_SF_F16.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         int16  y1 = GFLIB_IntegratorTR_w_F16(int16 u1, GFLIB_INTEGRATOR_TR_T_F16 work1[1], int16 work2[1], int16 u2, int16 u3, uint16 u4, int32 u5, int16 u6, int32 y2[1], int16 y3[1], int32 p1, int32 p2)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:02:56
 *
 */
...
#define S_FUNCTION_NAME  GFLIB_IntegratorTR_SF_F16
...
#include "gflib.h"
#include "GFLIB_IntegratorTR_w.h"
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GFLIB_INTEGRATOR_TR_T_F16", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GFLIB_IntegratorTRInit_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GFLIB_IntegratorTR_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_IntegratorTRInit_w_F16_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GFLIB_IntegratorTR_w_F16_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_IntegratorTR_SF_F16_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GFLIB_IntegratorTR_SF_F16_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GFLIB_INTEGRATOR_TR_T_F16Id);
        ...
        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F16.f32State */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F16Id, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F16.f16InK1 */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F16Id, 1);
        __dtBusInfo[3] = __dtSizeInfo[2];

        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F16.f16C1 */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F16Id, 2);
        __dtBusInfo[5] = __dtSizeInfo[2];

        /* Get information for accessing GFLIB_INTEGRATOR_TR_T_F16.u16NShift */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GFLIB_INTEGRATOR_TR_T_F16Id, 3);
        __dtBusInfo[7] = __dtSizeInfo[3];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GFLIB_INTEGRATOR_TR_T_F16* __work1BUS = (GFLIB_INTEGRATOR_TR_T_F16*)calloc(sizeof(GFLIB_INTEGRATOR_TR_T_F16), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GFLIB_INTEGRATOR_TR_T_F16* __work1BUS = (GFLIB_INTEGRATOR_TR_T_F16*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f32State, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__work1BUS[0].f16InK1, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);
        (void) memcpy(&__work1BUS[0].f16C1, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].u16NShift, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);

        /* Call the legacy code function */
        GFLIB_IntegratorTRInit_w_F16(*p3, *p4, __work1BUS, work2, *p2);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].f32State,  __dtBusInfo[1]);
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].f16InK1,  __dtBusInfo[3]);
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].f16C1,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].u16NShift,  __dtBusInfo[7]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GFLIB_IntegratorTR_w_F16(*u1, __work1BUS, work2, *u2, *u3, *u4, *u5, *u6, y2, y3, *p1, *p2);
