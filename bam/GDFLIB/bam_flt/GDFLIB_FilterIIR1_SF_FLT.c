/**
 * GDFLIB_FilterIIR1_SF_FLT.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         single y1 = GDFLIB_FilterIIR1_w_FLT(single u1, single u2, single u3, single u4, single u5, single u6, GDFLIB_FILTER_IIR1_T_FLT work1[1], single y2[1], single y3[1], int32 p1)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:00:30
 *
 */
...
#define S_FUNCTION_NAME  GDFLIB_FilterIIR1_SF_FLT
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
        ...
        ssRegisterTypeFromNamedObject(S, "GDFLIB_FILTER_IIR1_T_FLT", &dataTypeIdReg);
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GDFLIB_FilterIIR1Init_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GDFLIB_FilterIIR1_w_FLT", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GDFLIB_FilterIIR1Init_w_FLT_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GDFLIB_FilterIIR1_w_FLT_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GDFLIB_FilterIIR1_SF_FLT_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GDFLIB_FilterIIR1_SF_FLT_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
    ...
    if ((ssGetSimMode(S)!=SS_SIMMODE_SIZES_CALL_ONLY)) {

        {
            ...
            ssRegisterTypeFromNamedObject(S, "tFloat", &dataTypeIdReg);
            if(dataTypeIdReg == INVALID_DTYPE_ID) return;
        }
        CheckDataTypes(S);
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        DTypeId __GDFLIB_FILTER_IIR1_T_FLTId = ssGetDataTypeId(S, "GDFLIB_FILTER_IIR1_T_FLT");
        DTypeId __GDFLIB_FILTER_IIR1_COEFF_T_FLTId = ssGetDataTypeId(S, "GDFLIB_FILTER_IIR1_COEFF_T_FLT");
        DTypeId __singleId = ssGetDataTypeId(S, "single");

        /* Get common data type size */
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GDFLIB_FILTER_IIR1_T_FLTId);
        __dtSizeInfo[1] = ssGetDataTypeSize(S, __GDFLIB_FILTER_IIR1_COEFF_T_FLTId);
        __dtSizeInfo[2] = ssGetDataTypeSize(S, __singleId);

        /* Get information for accessing GDFLIB_FILTER_IIR1_T_FLT.trFiltCoeff.fltB0 */
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_T_FLTId, 0) + ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_COEFF_T_FLTId, 0);
        __dtBusInfo[1] = __dtSizeInfo[2];

        /* Get information for accessing GDFLIB_FILTER_IIR1_T_FLT.trFiltCoeff.fltB1 */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_T_FLTId, 0) + ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_COEFF_T_FLTId, 1);
        __dtBusInfo[3] = __dtSizeInfo[2];

        /* Get information for accessing GDFLIB_FILTER_IIR1_T_FLT.trFiltCoeff.fltA1 */
        __dtBusInfo[4] = ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_T_FLTId, 0) + ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_COEFF_T_FLTId, 2);
        __dtBusInfo[5] = __dtSizeInfo[2];

        /* Get information for accessing GDFLIB_FILTER_IIR1_T_FLT.fltFiltBufferX */
        __dtBusInfo[6] = ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_T_FLTId, 1);
        __dtBusInfo[7] = __dtSizeInfo[2];

        /* Get information for accessing GDFLIB_FILTER_IIR1_T_FLT.fltFiltBufferY */
        __dtBusInfo[8] = ssGetBusElementOffset(S, __GDFLIB_FILTER_IIR1_T_FLTId, 2);
        __dtBusInfo[9] = __dtSizeInfo[2];


        /* Alloc memory for the pwork 1 (__work1BUS) */
        {
            GDFLIB_FILTER_IIR1_T_FLT* __work1BUS = (GDFLIB_FILTER_IIR1_T_FLT*)calloc(sizeof(GDFLIB_FILTER_IIR1_T_FLT), ssGetDWorkWidth(S, 0));
            if (__work1BUS==NULL) {
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        tFloat* p2 = (tFloat*) ssGetRunTimeParamInfo(S, 1)->data;
        tFloat* p3 = (tFloat*) ssGetRunTimeParamInfo(S, 2)->data;
        tFloat* p4 = (tFloat*) ssGetRunTimeParamInfo(S, 3)->data;
        char* work1 = (char*) ssGetDWork(S, 0);

        GDFLIB_FILTER_IIR1_T_FLT* __work1BUS = (GDFLIB_FILTER_IIR1_T_FLT*) ssGetPWorkValue(S, 0);
        ...
        (void) memcpy(&__work1BUS[0].trFiltCoeff.fltA1, work1 +  __dtBusInfo[4],  __dtBusInfo[5]);
        (void) memcpy(&__work1BUS[0].fltFiltBufferX, work1 +  __dtBusInfo[6],  __dtBusInfo[7]);
        (void) memcpy(&__work1BUS[0].fltFiltBufferY, work1 +  __dtBusInfo[8],  __dtBusInfo[9]);

        /* Call the legacy code function */
        GDFLIB_FilterIIR1Init_w_FLT(*p2, *p3, *p4, __work1BUS);
        ...
        (void) memcpy(work1 +  __dtBusInfo[4], &__work1BUS[0].trFiltCoeff.fltA1,  __dtBusInfo[5]);
        (void) memcpy(work1 +  __dtBusInfo[6], &__work1BUS[0].fltFiltBufferX,  __dtBusInfo[7]);
        (void) memcpy(work1 +  __dtBusInfo[8], &__work1BUS[0].fltFiltBufferY,  __dtBusInfo[9]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GDFLIB_FilterIIR1_w_FLT(*u1, *u2, *u3, *u4, *u5, *u6, __work1BUS, y2, y3, *p1);
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    /* Verify AliasType/NumericType 'tFloat' specification */
    {
        uint32_T chk[] = {50814173, 4285868399, 2337721931, 3167949157};
        int_T status;
        status = CheckDataTypeChecksum(S, "tFloat", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink AliasType/NumericType 'tFloat'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink AliasType/NumericType 'tFloat' definition has changed since the S-Function was generated");
        }
    }

    /* Verify Bus/StructType 'GDFLIB_FILTER_IIR1_T_FLT', specification */
    {
        uint32_T chk[] = {567318826, 2057150671, 1104996925, 3235158738};
        int_T status;
        status = CheckDataTypeChecksum(S, "GDFLIB_FILTER_IIR1_T_FLT", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GDFLIB_FILTER_IIR1_T_FLT'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GDFLIB_FILTER_IIR1_T_FLT' definition has changed since the S-Function was generated");
        }
    }

    /* Verify Bus/StructType 'GDFLIB_FILTER_IIR1_COEFF_T_FLT', specification */
    {
        uint32_T chk[] = {1606572502, 2782269908, 132605617, 192878353};
        int_T status;
        status = CheckDataTypeChecksum(S, "GDFLIB_FILTER_IIR1_COEFF_T_FLT", &chk[0]);
        if (status==-1) {
          ssSetErrorStatus(S, "Unexpected error when checking the validity of the Simulink Bus/StructType 'GDFLIB_FILTER_IIR1_COEFF_T_FLT'");
        } else if (status==0) {
          ssSetErrorStatus(S, "The Simulink Bus/StructType 'GDFLIB_FILTER_IIR1_COEFF_T_FLT' definition has changed since the S-Function was generated");
        }
    }
}
