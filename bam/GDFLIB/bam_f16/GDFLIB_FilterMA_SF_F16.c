/**
 * GDFLIB_FilterMA_SF_F16.c
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         int16 y1 = GDFLIB_FilterMA_w_F16(int16 u1, GDFLIB_FILTER_MA_T_F16 work1[1], int16 work2[1], int16 u2, uint16 u3, int32 u4, int32 y2[1], int32 p1, int32 p2)
 *
 *    Simulink version           : 9.3 (R2019a) 23-Nov-2018
 *    C source code generated on : 15-Apr-2020 21:00:49
 *
 */
...
#define S_FUNCTION_NAME  GDFLIB_FilterMA_SF_F16
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        ssRegMdlInfo(S, "GDFLIB_FilterMAInit_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for  */
        ssRegMdlInfo(S, "GDFLIB_FilterMA_w_F16", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

        /* Register reserved identifier for wrappers */
        if (isSimulationTarget) {

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GDFLIB_FilterMAInit_w_F16_wrapper_InitializeConditions", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for  */
            ssRegMdlInfo(S, "GDFLIB_FilterMA_w_F16_wrapper_Output", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for allocating PWork for SimulationTarget */
            ssRegMdlInfo(S, "GDFLIB_FilterMA_SF_F16_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "GDFLIB_FilterMA_SF_F16_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        __dtSizeInfo[0] = ssGetDataTypeSize(S, __GDFLIB_FILTER_MA_T_F16Id);
        ...
        __dtBusInfo[0] = ssGetBusElementOffset(S, __GDFLIB_FILTER_MA_T_F16Id, 0);
        __dtBusInfo[1] = __dtSizeInfo[1];

        /* Get information for accessing GDFLIB_FILTER_MA_T_F16.u16NSamples */
        __dtBusInfo[2] = ssGetBusElementOffset(S, __GDFLIB_FILTER_MA_T_F16Id, 1);
...
static void mdlInitializeConditions(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        GDFLIB_FILTER_MA_T_F16* __work1BUS = (GDFLIB_FILTER_MA_T_F16*) ssGetPWorkValue(S, 0);

        /* Assign the Simulink structure work1 to user structure __work1BUS */
        (void) memcpy(&__work1BUS[0].f32Acc, work1 +  __dtBusInfo[0],  __dtBusInfo[1]);
        (void) memcpy(&__work1BUS[0].u16NSamples, work1 +  __dtBusInfo[2],  __dtBusInfo[3]);

        /* Call the legacy code function */
        GDFLIB_FilterMAInit_w_F16(*p3, __work1BUS, work2, *p2);

        /* Assign the user structure __work1BUS to the Simulink structure work1 */
        (void) memcpy(work1 +  __dtBusInfo[0], &__work1BUS[0].f32Acc,  __dtBusInfo[1]);
        (void) memcpy(work1 +  __dtBusInfo[2], &__work1BUS[0].u16NSamples,  __dtBusInfo[3]);
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Call the legacy code function */
        *y1 = GDFLIB_FilterMA_w_F16(*u1, __work1BUS, work2, *u2, *u3, *u4, y2, *p1, *p2);
